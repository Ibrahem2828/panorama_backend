...
| V6 Cryptography | Required field-encryption key and private local named-volume storage with protected API streams | `config/settings/production.py`, `config/settings/storage.py` | Verified by production-settings/storage tests; host encryption is operational |
...
| V12 Files | Private named-volume storage, randomized object names, authenticated ticket-owner streams, and restrictive cache headers | files/printing/support/verification views | Verified by tests/configuration; reverse-proxy non-exposure is operational |
...
| V14 Configuration | Production fails fast for required secrets, DB, Redis, origins, mail, and a persistent local media path | `config/settings/production.py` | Verified by tests |
...
Open operational items: rotate potentially exposed secrets, configure malware
scanning, and run staging restore/load/penetration tests. Do not delete the
production media Volume as part of deployment or cleanup.
